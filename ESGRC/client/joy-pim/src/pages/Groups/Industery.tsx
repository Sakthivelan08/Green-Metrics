...
import { columnHeader, PIMHearderText, PIMsearchBoxStyles } from '../PimStyles';
import { SearchBox } from 'office-ui-fabric-react/lib/SearchBox';
import { Link } from 'react-router-dom';

class Industery extends React.Component<any, any> {
  ...
  async refresh() {
    try {
      ...
      const apiResponse1: any = (await this.apiClient.getIndustry()).result;
      this.setState({
    ...
    return (
      <div className="layout width-100">
        <div className="bg-Color">
          <Link to={`/metrics/industry`} className="headerText">
            {t('MENU_METRICS')}/{t('SUBMENU_INDUSTRY')}
          </Link>
          <Grid
            item
            container
...
const ComponentTranslated: any = withTranslation()(Industery);
