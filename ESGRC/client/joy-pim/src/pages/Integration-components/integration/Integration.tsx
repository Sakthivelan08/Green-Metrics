import { AddFam, columnHeader, customStyles, PIMHearderText } from '@/pages/PimStyles';
...
export class Integration extends React.Component<any, State> {
    ...
    getTableData(list?: number[]): void {
        ...
        const res = Promise.all(list.map(async e => {
            let response;
            if (e === 1) {
                response = await this.apiClient.getApiIntegration();
            }
            return { e, response };
        }));
    ...
    render() {
        ...
        const _column: TableColumn<any>[] = [
            ...
            {
                name: <div className={columnHeader}>{`${t('LABEL_PARAMETER')}`}</div>,
                selector: (row: any) => row?.parameter,
            },
            {
                name: <div className={columnHeader}>{`${t('LABEL_PATH')}`}</div>,
                selector: (row: any) => row?.path,
            },
            {
                name: <div className={columnHeader}>{`${t('LABEL_TYPE')}`}</div>,
