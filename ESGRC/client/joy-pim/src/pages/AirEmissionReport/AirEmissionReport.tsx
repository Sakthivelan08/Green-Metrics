...
import AddOrUpdateReports from '@/Reports/AddorUpdateReports';

class AirEmissionReport extends React.Component<any, any> {
  ...
  async refresh() {
    try {
      ...
      const apiResponse = await this.apiClient.getAllAirEmissionPdfReports();
    ...
    return (
      <div className="layout width-100">
        <div className="bg-Color">
          <Link to={`/activity/AirEmissionReport`} className="headerText">
            {t('MENU_ACTIVITY')}/ {t('SUBMENU_AIR_EMISSION_REPORT')}
          </Link>
          <Grid
            item
            container
...
const ComponentTranslated = withTranslation()(AirEmissionReport);
